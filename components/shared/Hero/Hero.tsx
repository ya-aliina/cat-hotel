// import Image from 'next/image';

// import { PawButton } from '@/components/ui/PawButton';

// const Hero = () => {
//   return (
//     <section className="relative w-full">
//       {/* desktop */}
//       <Image
//         src="/hero.png"
//         alt="Котейка"
//         width={1920}
//         height={800}
//         priority
//         className="absolute hidden w-full h-auto sm:block"
//       />

//       {/* tablet + mobile */}
//       <Image
//         src="/hero-mobile.png"
//         alt="Котейка"
//         width={1024}
//         height={700}
//         priority
//         className="absolute block w-full h-auto sm:hidden"
//       />

//       {/* <div className="relative max-w-[1170px] md:p-30 flex">
//         <div className="space-y-4 z-10 max-md:mx-auto max-md:pt-10">
//           <p>м.Київ</p>
//           <h1 className="text-5xl font-bold">Котейка</h1>
//           <p>Затишний готель для котів і кішок</p>
//           <PawButton className="">Забронювати</PawButton>
//         </div>
//       </div> */}
//       <div className="relative max-w-[1170px] h-full my-auto ">
//         <div className="space-y-4 z-10 max-md:mx-auto max-md:pt-10">
//           <p>м.Київ</p>
//           <h1 className="text-5xl font-bold">Котейка</h1>
//           <p>Затишний готель для котів і кішок</p>
//           <PawButton className="max-sm:mx-auto">Забронювати</PawButton>
//         </div>
//       </div>
//     </section>
//   );
// };

// export default Hero;

import Image from 'next/image';

import { PawButton } from '@/components/ui/PawButton';

const Hero = () => {
  return (
    <section className="relative w-full overflow-hidden">
      <div className="relative w-full min-h-[550px] sm:h-[512px]">
        {/* Фоновое изображение */}
        <picture>
          <source media="(min-width: 640px)" srcSet="/hero.png" />
          <Image
            src="/hero-mobile.png"
            alt="Котейка"
            fill
            priority
            className="object-cover object-center sm:object-[center_20%]"
          />
        </picture>

        {/* Контентная область */}
        <div className="absolute inset-0 flex items-start sm:items-center">
          <div className="container mx-auto px-4 md:px-10 lg:px-[135px]">
            <div className="flex flex-col items-center sm:items-start text-center sm:text-left pt-12 sm:pt-0">
              <p className="text-sm md:text-base font-medium mb-2 opacity-90">м. Київ</p>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-4 tracking-tight">
                Котейка
              </h1>
              <p className="text-lg md:text-xl mb-8 max-w-[260px] sm:max-w-md leading-relaxed">
                Затишний готель для котів і кішок
              </p>
              <div className="w-full sm:w-auto">
                <PawButton className="mx-auto sm:mx-0">Забронювати</PawButton>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Hero;
